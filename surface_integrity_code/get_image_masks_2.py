...
from read_utils import get_rgb, get_depth, get_mask, get_depth_confidence, get_intrinsics, get_pose_matrix, get_name
...
def load_frame(row: pd.Series) -> Frame:
    ...
    name = get_name(row)
    frame = Frame(color_image, depth_image, mask_image, intrinsics, pose_matrix, name=name)
    return frame
...
def fuse_patches(patches, original_image_size, patch_size):
    ...
    patch_rows = H // actual_ph
    patch_cols = W // actual_pw
...
def process_frame(frame: Frame, model=None):
    # Save original color image for reference
    color_image = frame.color_image
    color_image = cv2.cvtColor(color_image, cv2.COLOR_RGBA2BGRA) if color_image.shape[2] == 4 else cv2.cvtColor(color_image, cv2.COLOR_RGB2BGR)
    cv2.imwrite(os.path.join(OUTPUT_PATH, f"{frame.name}_color_image.png"), color_image)
    ...
    cv2.imwrite(os.path.join(OUTPUT_PATH, f"{frame.name}_warped_fused_output.png"), fused_image)
    cv2.imwrite(os.path.join(OUTPUT_PATH, f"{frame.name}_warped_fused_pred_mask.png"), fused_pred_mask)
    ...
    cv2.imwrite(os.path.join(OUTPUT_PATH, f"{frame.name}_fused_output.png"), unwarped_image)
    ...
    cv2.imwrite(os.path.join(OUTPUT_PATH, f"{frame.name}_fused_pred_mask.png"), unwarped_pred_mask)
